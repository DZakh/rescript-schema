...
function parse(input, raise) {
  var match = typeof input;
  switch (match) {
    case "number" :
        if (Number.isNaN(input)) {
          return Curry._1(raise, input);
        } else {
          return input;
        }
    case "object" :
        ...
        if (Array.isArray(input)) {
          ...
          for(var idx = 0 ,idx_finish = input.length; idx < idx_finish; ++idx){
            ...
            output.push(parse(inputItem, raise));
          }
          return output;
        }
        ...
        for(var idx$1 = 0 ,idx_finish$1 = keys.length; idx$1 < idx_finish$1; ++idx$1){
          var key = keys[idx$1];
          var field = input[key];
          output$1[key] = parse(field, raise);
        }
        return output$1;
    case "boolean" :
    case "string" :
...
function raiseUnexpectedTypeError(input, struct) {
  ...
  if (typeof number !== "object") {
    ...
    switch (number.TAG) {
      case "JSNumber" :
          tmp = Number.isNaN(number._0) ? "NaN" : "Float";
          break;
      case "JSString" :
          tmp = "String";
          break;
...
function validateJsonableStruct(_struct, rootStruct, _isRootOpt, _param) {
  while(true) {
    ...
    switch (unknown.TAG) {
      case "Literal" :
          parse(unknown._0, (function(struct){
      ...
      case "Object" :
          var fieldNames = unknown.fieldNames;
          var fields = unknown.fields;
          ...
          return ;
      case "Tuple" :
          unknown._0.forEach(function (childStruct, i) {
                try {
          ...
          return ;
      case "Union" :
          unknown._0.forEach(function (childStruct) {
...
function internalToInlinedValue(_struct) {
  while(true) {
    ...
    switch (unknown.TAG) {
      ...
      case "Object" :
          var fields = unknown.fields;
          return "{" + unknown.fieldNames.map((function(fields){
                      return function (fieldName) {
                        return JSON.stringify(fieldName) + ":" + internalToInlinedValue(fields[fieldName]);
                      }
                      }(fields))).join(",") + "}";
      case "Tuple" :
          return "[" + unknown._0.map(internalToInlinedValue).join(",") + "]";
      case "Union" :
          _struct = unknown._0[0];
          continue ;
      default:
        throw undefined;
    }
...
function factory$1(literal, variant) {
  var tagged = {
    TAG: "Literal",
    _0: literal
  };
  ...
  return {
          ...
          pf: (function (ctx) {
              planSyncTransformation(ctx, (function (input) {
                      if (Caml_obj.equal(input, literal)) {
                        return variant;
                      } else {
...
function factory$2(literal) {
  return factory$1(literal, literal);
}
...
function empty$1(param) {
  return factory$1(null, undefined);
}
...
function parseTransformationFactory$4(ctx) {
  planSyncTransformation(ctx, (function (input) {
          return parse(input, (function (input) {
                        return raiseUnexpectedTypeError(input, ctx.s);
                      }));
        }));
}
...
function internalInline(struct, maybeVariant, param) {
  ...
  if (typeof value !== "object") {
    ...
    switch (value.TAG) {
      case "Literal" :
          ...
          inlinedStruct = maybeVariant !== undefined ? "S.literalVariant(" + (
              value$1 === undefined ? "undefined" : JSON.stringify(value$1)
            ) + ", " + maybeVariant + ")" : "S.literal(" + (
              value$1 === undefined ? "undefined" : JSON.stringify(value$1)
            ) + ")";
          break;
      case "Option" :
          var inlinedInnerStruct = internalInline(value._0, undefined, undefined);
          ...
          break;
      case "Null" :
          inlinedStruct = "S.null(" + internalInline(value._0, undefined, undefined) + ")";
          break;
      case "Array" :
          inlinedStruct = "S.array(" + internalInline(value._0, undefined, undefined) + ")";
          break;
      case "Object" :
      ...
      case "Union" :
          ...
          inlinedStruct = "S.union([" + value._0.map(function (s) {
          ...
          break;
      case "Dict" :
          inlinedStruct = "S.dict(" + internalInline(value._0, undefined, undefined) + ")";
          break;
  ...
  var refinements$4 = refinements(struct);
  var inlinedStruct$4 = refinements$4.length !== 0 ? (Js_dict.unsafeDeleteKey(metadataMap, metadataId$1), inlinedStruct$3 + refinements$4.map(function (refinement) {
            ...
            }).join("")) : inlinedStruct$3;
  var refinements$5 = refinements$1(struct);
  var inlinedStruct$5 = refinements$5.length !== 0 ? (Js_dict.unsafeDeleteKey(metadataMap, metadataId$2), inlinedStruct$4 + refinements$5.map(function (refinement) {
              var match = refinement.kind;
              if (typeof match !== "object") {
            ...
            }).join("")) : inlinedStruct$4;
  var refinements$6 = refinements$2(struct);
  var inlinedStruct$6 = refinements$6.length !== 0 ? (Js_dict.unsafeDeleteKey(metadataMap, metadataId$3), inlinedStruct$5 + refinements$6.map(function (refinement) {
              var match = refinement.kind;
              if (match.TAG === "Min") {
            ...
            }).join("")) : inlinedStruct$5;
  var refinements$7 = refinements$3(struct);
  var inlinedStruct$7 = refinements$7.length !== 0 ? (Js_dict.unsafeDeleteKey(metadataMap, metadataId$4), inlinedStruct$6 + refinements$7.map(function (refinement) {
              var match = refinement.kind;
              switch (match.TAG) {
                case "Min" :
            ...
            }).join("")) : inlinedStruct$6;
  var inlinedStruct$8 = Object.keys(metadataMap).length !== 0 ? "{\n  let s = " + inlinedStruct$7 + "\n  let _ = %raw(\`s.m = " + JSON.stringify(metadataMap) + "\`)\n  s\n}" : inlinedStruct$7;
  var match$1 = struct.t;
  if (typeof match$1 === "object" && match$1.TAG === "Literal") {
    return inlinedStruct$8;
  }
  if (maybeVariant !== undefined) {
    return inlinedStruct$8 + ("->S.variant(v => " + maybeVariant + "(v))");
  } else {
    return inlinedStruct$8;
  }
}
