...
let map_type_decl decl =
  ...
  match
    (getAttributeByName ptype_attributes "schema", ptype_manifest, ptype_kind)
  with
  | Ok None, _, _ -> []
  ...
  | Ok _, Some { ptyp_desc = Ptyp_variant (row_fields, _, _) }, Ptype_abstract
    ->
      generate_decls type_name (Polyvariants.generateSchemaExpression row_fields)
  | Ok _, Some manifest, _ ->
      generate_decls type_name (Codecs.generateSchemaExpression manifest)
  | Ok _, None, Ptype_variant decls ->
      generate_decls type_name (Variants.generateSchemaExpression decls)
  | Ok _, None, Ptype_record decls ->
      generate_decls type_name (Records.generateSchemaExpression decls)
